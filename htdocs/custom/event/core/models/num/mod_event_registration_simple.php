...
/**
 *	\file       htdocs/event/models/mod_event_simple.php
 *	\ingroup    event
...
/**
 * 	\class      mod_event_simple
 * 	\brief      Class to manage the numbering module Simple for event references
 */
class mod_event_registration_simple extends ModeleNumRefRegistration
{
	var $version='dolibarr';		// 'development', 'experimental', 'dolibarr'
	var $prefix='RE';
    var $error='';
	var $nom = "Simple";
	function mod_event_registration_simple() {
		return 1;
	}
	...
	*/
    function getNextValue($objsoc=0,$event='')
    {
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."event_registration";
		...
		{
			dol_syslog("mod_event_registration_simple::getNextValue sql=".$sql);
			return -1;
		}

		$date=empty($event->date_c)?dol_now():$event->date_c;

		$yymm = strftime("%y%m",$date);
		$num = sprintf("%04s",$max+1);

		dol_syslog("mod_event_simple::getNextValue return ".$this->prefix.$yymm."-".$num);
