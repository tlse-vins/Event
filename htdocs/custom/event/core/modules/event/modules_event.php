 ...
 */
abstract class ModeleEvent extends CommonDocGenerator
{
	var $error='';
 ...
 */
 function event_ics_create($db, $object, $model = 'registration',$outputlangs)
 {
 	global $conf,$langs;
     ...
     $ics.= "BEGIN:VEVENT\n";
     $ics.= "DTSTART:".date("Ymd", $object->date_event)."T".str_replace(":", "", $object->time_start)."\n";
     $ics.= "DTEND:".date("Ymd", $object->date_event)."T".str_replace(":", "", $object->time_end)."\n";
     $ics.= "SUMMARY:".$object->label."\n";
     $chaine = strip_tags(html_entity_decode($object->description));
     $chaine = str_replace("\n"," ",$chaine);
   ...
   if (!file_exists("./../ics/"))
       mkdir("./../ics");
   $file = DOL_DOCUMENT_ROOT."/custom/event/ics/event_".$object->ref.".ics";
 ...
 *      @return     int          0 si KO, 1 si OK
 */
function event_pdf_create($db, $object, $model,$outputlangs)
{
	global $conf,$langs;
	$langs->load("event@event");
	...
	if (! dol_strlen($model))
	{
		...
		{
			$model='registration';
			print $langs->trans("Error")." ".$langs->trans("Error_EVENT_REGISTRATION_ADDON_PDF_NotDefined");
			return 0;
		}
	}
 ...
 * @return  int
 */
function event_delete_preview($db, $objectid)
{
	global $langs,$conf;
